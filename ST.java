...
public class ST extends Server {
    ...
    public void toCsv(String[] lista) throws Exception {
        FileWriter writer = new FileWriter("C:\\Users\\onuno\\Documents\\GitHub\\new\\service-discovery-java\\rmi.csv", true);
    ...
    public static void csvToString() {
        ...
        File file= new File("C:\\Users\\onuno\\Documents\\GitHub\\new\\service-discovery-java\\rmi.csv");
        ...
        while (true) {
            try {
                ...
                if (lista_respostas[2].equals("1")) {
                    ...
                    // FALTA METER BONITO E OBVIAMENTE PASSAR PARA O LADO DO CLIENTE, DA PARA ENVIAR ARRAYLIST MULTIDIMENSIONAL POR SOCKET'
                    if(lista_respostas[3].equals("1")){
                        ...
                        for(int i=1;i<=lines.size();i++){
                            if(lines.get(i-1).get(2).equals("1")){
                                System.out.println(i + "-IP: " + lines.get(i-1).get(3) + " PORT: " + lines.get(i-1).get(4));
                        ...
                        for(int i=1;i<=lines.size();i++){
                            if(lines.get(i-1).get(2).equals("2")){
                                System.out.println(i + "-IP: " + lines.get(i-1).get(3) + " PORT: " + lines.get(i-1).get(4) + " Nome: " + lines.get(i-1).get(5));
                                System.out.println("  Descrição: " + lines.get(i-1).get(1));
                                System.out.println("  Autor do Registo: " + lines.get(i-1).get(0));
                                System.out.println("\n");
                            }
                    ...
                    toCsv(lista_csv);
                    //System.out.print("Registo efetuado com sucesso.\nPrima qualquer tecla para terminar a execução do programa. ");
                    //System.in.read();
                    //System.exit(0);
                }
